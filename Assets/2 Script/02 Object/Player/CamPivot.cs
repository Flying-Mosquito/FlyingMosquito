...
public class CamPivot : TimeAffectedObj//MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player = GetComponentInParent<PlayerCtrl>();//GameObject.FindObjectOfType<PlayerCtrl>();//GameObject.Find("Player").GetComponent<PlayerCtrl>();// PlayerCtrl.Instance;//GameManager.Instance.PlayerCtrl;//GameObject.Find("Player").GetComponent<Player>();
    ...
    private void Move_RightLeft()   // 카메라효과 - 좌우, 상하로 움직이기
    {
        ...
        Target_fSpeed = player.fSpeed;// * 0.3f;
        if (player.state != Constants.ST_CLING)        // 붙어있지 않을 때의 카메라 효과(위치)
        {


            if (player.state == Constants.ST_BOOST)    // BOOST중이면 카메라가 움직이는 효과를 주고, 아니라면 원점으로 돌아가게 한다
            {
        ...
        // 여기도 수정?
        if (player.state == Constants.ST_BOOST)        // 부스터 사용할 때의 카메라 회전값 설정
        {
            //print("부스터 쓰는중");
    ...
    {
        // 충격관련된 수학 - 아마 sin cos
        if ((player.variable & Constants.BV_bStun) > 0)//_Player.isConfused) // 플레이어가 Confused 상태라면 흔들어주세요 ( 충돌 후 ) 
        {
            fX += 0.1f;
            // 여기서 흔들흔들
            tr.localPosition = Vector3.Lerp(tr.localPosition
            ...
            if (player.fSpeed > fPower)  // 더 큰 충격을 받았을 때? 
            {
                fPower = player.fSpeed;
